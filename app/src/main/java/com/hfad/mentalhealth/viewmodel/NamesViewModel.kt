...
class NamesViewModel @Inject constructor(private val repository: NamesRepository): ViewModel() {
    ...
    fun getAllNames() {
        viewModelScope.launch {
            ...
            if (response.isSuccessful) {
                _names.value = (response.body() as UserData).users
            } else {
    ...
    private fun convertToDoctorList(names: Names, position: Int): DoctorList {
        val doctorList = hardcodedList.getOrNull(position)
        return doctorList?.copy(
    ...
    fun getDoctorList(): List<DoctorList> {
        ...
        return apiData.mapIndexed { index, names -> convertToDoctorList(names, index) }
    }
}
